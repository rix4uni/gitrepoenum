...
import (
	...
	"github.com/rix4uni/gitrepoenum/banner"
)
...
func checkAndUpdateTool() {
	...
	fmt.Printf("Updating gitrepoenum from version %s to %s...\n", currentVersion, latestVersion)
	cmd := exec.Command("go", "install", "github.com/rix4uni/gitrepoenum@latest")
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr

	err = cmd.Run()
	if err != nil {
		fmt.Println("Error updating gitrepoenum:", err)
		os.Exit(1)
	}

	fmt.Println("gitrepoenum has been updated to the latest version.")
}

// Function to get the current version by executing 'gitrepoenum -v'
func getCurrentVersion() (string, error) {
	cmd := exec.Command("gitrepoenum", "-v")
	...
	re := regexp.MustCompile(`Current gitrepoenum version (v[0-9]+\.[0-9]+\.[0-9]+)`)
...
func getLatestVersion() (string, error) {
	...
	resp, err := http.Get("https://raw.githubusercontent.com/rix4uni/gitrepoenum/refs/heads/main/banner/banner.go")
	if err != nil {
		return "", err
	}
...
func init() {
	// Define flags
	rootCmd.Flags().BoolP("update", "u", false, "update gitrepoenum to latest version")
